...
Varyings ShadowCasterPassVertex(Attributes _input)
{
    Varyings output;
...
void ShadowCasterPassFragment(Varyings _input)
{
    UNITY_SETUP_INSTANCE_ID(_input);
    ...
    float4 base = GetBase(config);
		...
		float dither = InterleavedGradientNoise(_input.positionCS_SS.xy, 0);
		clip(base.a - dither);
    #endif
}
